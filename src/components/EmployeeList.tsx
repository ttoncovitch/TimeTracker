...
export function EmployeeList({
  ...
  const filtered = useMemo(() => {
    return summaries
      .filter((s) => {
        const matchesSearch = normalizeName(s.employeeName).includes(
          normalizeName(searchTerm),
        );
        const matchesLob = selectedLob === "ALL" || s.lob === selectedLob;
        const matchesLang =
          selectedLang === "ALL" ||
          s.language?.toUpperCase().trim() === selectedLang;
        const matchesTL =
          selectedTL === "ALL" || s.supervisor?.trim() === selectedTL;
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2">
            {(() => {
              const isAnyStatusFilterActive =
                ...
                globalIncludeShort30Min ||
                globalIncludeCheck ||
                globalIncludeAbsences;              if (!isAnyStatusFilterActive) return null;
              ...
              const triggerEmail = (sendToOps: boolean) => {
                ...
                let modName = "";
                if (selectedTL !== "ALL") {
                  const agentNames = filtered.map((s) => s.employeeName).filter(Boolean);
                  modName = agentNames.length > 0 ? agentNames.join(", ") : "AGENTS";
                ...
                <>
                  {showSendToOpsConfirm ? (
                    <div className="h-11 bg-emerald-50 border border-emerald-200 rounded-xl px-3 inline-flex items-center gap-2 shadow-sm animate-in fade-in zoom-in-95 duration-200">
                      <span className="text-xs font-semibold text-emerald-800">Send to ops?</span>
                      <button
                        onClick={() => {
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-slate-50/95 backdrop-blur border-b border-slate-200 text-[10px] uppercase font-black text-slate-500 tracking-widest sticky top-0 z-30 outline outline-1 outline-slate-200 shadow-sm">
              <tr>
                ...
                >
                  {String(t("absencesString") || "").toUpperCase()}{" "}
                  {sortBy === "absences" && (
                    <span className="text-[10px] ml-1">
                      {sortDirection === "asc" ? "↑" : "↓"}
                    </span>
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      ...
                      title={
                        (s.totalAbsences || 0) > 0
                          ? `${s.totalAbsences} ${t("absencesString").toLowerCase()} no período`
                          : t("withoutRecords")
                      }
                    >
                      <span
                        className={`inline-flex items-center justify-center text-[11px] px-1.5 py-0.5 rounded transition-colors ${(s.totalAbsences || 0) > 0 ? "bg-red-100 text-red-700 font-black border border-red-200" : "text-slate-300 font-bold"}`}
                      >
