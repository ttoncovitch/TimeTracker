                ...
                {filtered.map((s, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-2 text-center" title={nonModTotal > 0 ? `${nonModTotal}m em N.M` : '0m em N.M'}>
                  ...
                  {!isWcOnly && (
                    <div className="flex bg-slate-800 rounded-md p-1 border border-slate-700 w-full sm:w-auto overflow-x-auto gap-1">
                      <button
                        ...
                        className={`px-2 py-1 shrink-0 rounded text-[9px] font-black uppercase tracking-wider transition-colors w-full sm:w-auto ${onlyExceptions ? 'bg-rose-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-700 hover:text-slate-300'}`}
                      >
                        {t('exceptions')}
                      </button>
                      <button
                        onClick={() => setIncludeWc(!includeWc)}
                        className={`px-2 py-1 shrink-0 rounded text-[9px] font-black uppercase tracking-wider transition-colors w-full sm:w-auto ${includeWc ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-700 hover:text-slate-300'}`}
                      >
                        Organic
                      </button>
                      <button
                        onClick={() => setFilterNm(!filterNm)}
                        className={`px-2 py-1 shrink-0 rounded text-[9px] font-black uppercase tracking-wider transition-colors w-full sm:w-auto ${filterNm ? 'bg-teal-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-700 hover:text-slate-300'}`}
                      >
                        N.M
                      </button>
                      <button
                      ...
                      <div className="flex items-center gap-1 bg-slate-700/50 px-2 py-1 shrink-0 rounded ml-1 border border-slate-600/50">
                         <span className="text-[9px] font-bold tracking-widest text-slate-400 uppercase">TOTAL</span>
    ...
    return (
        <div className={`flex flex-col p-5 border rounded-2xl shadow-sm transition-all relative overflow-hidden bg-white ${isHighlighted ? (filterNm ? 'border-teal-200 ring-teal-50/50 ring-4' : isWcOnly ? 'border-amber-200 ring-amber-50/50 ring-4' : isIdleOnly ? 'border-rose-200 ring-rose-50/50 ring-4' : 'border-rose-200 ring-rose-50/50 ring-4') : 'border-slate-100'}`}>
                            ...
                            return (
                                <div key={i} className={`flex items-center px-2 py-1 rounded text-[10px] border gap-1.5 shadow-sm ${isOverbreak ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200'}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${
                                        ...
                                        b.type === 'non_moderating' ? 'bg-teal-500' :
                                        'bg-slate-400'
                                    }`} />
            ...
            {visibleBreaks.some(b => {
                ...
                <div className="mt-4 bg-emerald-50/50 rounded-xl p-3 border border-emerald-100">
                    <p className="text-[9px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2">Anotações do Agente</p>
                    <div className="space-y-2">
                        {visibleBreaks
                            ...
                            .map((b, idx) => (
                                <div key={idx} className="bg-white p-2 rounded border border-emerald-100 text-[11px]">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-bold text-emerald-700">{format(b.startTime, 'HH:mm')} - {format(b.endTime, 'HH:mm')}</span>
                                        <span className="text-[10px] text-slate-400 font-medium">({b.durationMinutes}m)</span>
                                    </div>
                                    <p className="italic text-slate-600">"{b.remarks}"</p>
                                </div>
                            ))
                        }
                    </div>
