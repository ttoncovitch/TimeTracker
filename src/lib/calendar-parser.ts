    ...
    reader.onload = (e) => {
      try {
        ...
        for (const sheetName of workbook.SheetNames) {
           ...
           // We are looking for a row where column F (index 5) or nearby starts looking like dates or days
           let headerRowIdx = -1;
           for (let i = 0; i < Math.min(20, rows.length); i++) {
              if (rows[i] && rows[i].length > 4) {
                 ...
                 // If the row contains "name" or something similar in col B, it's a strong header candidate
              ...
              for (let i = headerRowIdx + 1; i < rows.length; i++) {
                 ...
                 for (let col = 5; col < Math.max(headers.length, row.length); col++) {
                    ...
                    if (typeof headerVal === 'number') {
                       ...
                       dateKey = `${String(d.getUTCDate()).padStart(2, '0')}/${String(d.getUTCMonth() + 1).padStart(2, '0')}/${d.getUTCFullYear()}`;
                    }

                    const shiftVal = String(row[col] || '').trim();
                    if (shiftVal && shiftVal !== '-' && shiftVal.toLowerCase() !== 'vazio') {
                       schedule[dateKey] = shiftVal;
                       // Add a short MM/dd / dd/MM variation to improve matching chances
                       const parts = dateKey.split('/');
                       if (parts.length >= 2) {
                          schedule[`${parts[0]}/${parts[1]}`] = shiftVal;
                          schedule[`${parts[2]}-${parts[1]}-${parts[0]}`] = shiftVal; // YYYY-MM-DD
                       }
                       if (!firstValidShift && shiftVal.toLowerCase() !== 'off') firstValidShift = shiftVal;
                    }
                 }
                 ...
                 if (!seenNames.has(normName)) {
                     ...
                     calendarData.push({
                       email: '', // mostly no emails in this layout
                       name: name,
                       lob: lob,
                       language: language,
                       supervisor: '', 
                       shift: firstValidShift || '', // take first non-off shift as general shift
                       schedule
                     });
                 }
              }
           } else {
             // Fallback to JSON keys if headers couldn't be inferred by positions
